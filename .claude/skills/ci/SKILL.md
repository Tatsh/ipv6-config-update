...
- **changelog** - update `CHANGELOG.md` with entries for the changes.
  ...
  files under `ipv6_config_update/`, `src/`,
...
For example, if a commit contains `ipv6_config_update/src/main.cpp`, and
`CHANGELOG.md`, the component is determined by
...
- Header file `include/ipv6_config_update/util.h` → `util:`.
